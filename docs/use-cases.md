...
1. Log in
2. Browse active tests (enabled, within availability window)
3. Start test — system creates a result record, selects and optionally shuffles questions
4. Answer questions:
   - **One-per-page**: submit answer, see next
   - **All-on-page**: answer everything, submit once
5. View results — score (%) and grade (1–5)
6. Review answers (if enabled) — per question: your answer vs correct answer

![Test list](images/screenshots/09-student-test-list.png)

#### One-per-page mode

Each question shown individually. Per-question time tracked.
...
**Session loss**: test state lives in PHP session. Session expiry = lost progress. No recovery.
...
|------|------|-------|---------|
| 1 | Free text | Textarea | All or nothing (exact match) |
| 2 | Ordering | Position dropdowns | All or nothing |
| 3 | Single choice | Radio buttons | All or nothing |
| 4 | Multiple selection | Checkboxes | Partial credit: 100%/N per correct pick |
